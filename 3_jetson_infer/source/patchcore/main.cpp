// ---------------------------------------------------------------------------
// main.cpp - PatchCore inference (Jetson Orin) with TensorRT 10.3 + FAISS-GPU
// ---------------------------------------------------------------------------
#include <NvInfer.h>
...
public:
    explicit TrtCtx(const std::string& plan) {
        ...
        // find the biggest output tensor (feature map)
        int nbIO = engine_->getNbIOTensors();
        size_t maxElems = 0;
        for (int i = 0; i < nbIO; ++i) {
            const char* n = engine_->getIOTensorName(i);
            if (!n || std::string(n) == inName_) continue;
            auto s = engine_->getTensorShape(n);
            size_t elems = std::accumulate(s.d, s.d + s.nbDims, 1LL,
                                           std::multiplies<int64_t>());
            if (elems > maxElems) { maxElems = elems; outName_ = n; }
        }
        outSize_ = maxElems;

        // collect all binding names
        for (int i = 0; i < nbIO; ++i)
            if (const char* n = engine_->getIOTensorName(i)) tensorNames_.emplace_back(n);
...
private:
    ...
    void* dIn_{nullptr}; void* dOut_{nullptr};
    cudaStream_t stream_{};
};

// ---------------- memory-bank -> FAISS GPU ----------------
std::unique_ptr<faiss::gpu::GpuIndexFlatL2>
load_bank(const std::string& path)
{
    cnpy::npz_t npz = cnpy::npz_load(path);
    auto it = npz.find("embeddings");
    if (it == npz.end()) throw std::runtime_error("'embeddings' not found in npz");
    cnpy::NpyArray& arr = it->second;

    static faiss::gpu::StandardGpuResources res;
    auto index = std::make_unique<faiss::gpu::GpuIndexFlatL2>(&res, arr.shape[1]);
    index->add(arr.shape[0], arr.data<float>());
    return index;
}
...
int main(int argc, char** argv)
{
    try {
        ...
        // ---- command-line ----
        cxxopts::Options opt("patchcore",
            "PatchCore inference (raw score) with per-stage timing");
        opt.add_options()
            ("e,engine",       "TensorRT engine (.plan)",    cxxopts::value<std::string>())
            ("m,memory_bank",  "memory_bank.npz",            cxxopts::value<std::string>())
        ...
        if (a.count("help") || !a.count("engine") || !a.count("memory_bank")
            || !a.count("data") || !a.count("image_size") || !a.count("threshold"))
        {
            std::cout << opt.help() << std::endl; return 0;
        }
        const std::string engine_path = a["engine"].as<std::string>();
        const std::string bank_path   = a["memory_bank"].as<std::string>();
        ...
        std::ofstream csv("result_patchcore.csv");
        ...
        for (auto& f : images) {
            ...
            std::vector<float> input(img.total()*3);
            for (int c=0;c<3;++c)
                std::memcpy(input.data() + c*img.total(),
                            ch[c].data, img.total()*sizeof(float));
            prep_ms += t.toc();

            // inference + kNN
            t.tic();
            trt.infer(input.data(), feats.data());
            std::vector<float> dist(P); std::vector<faiss::idx_t> ids(P);
            index->search(P, feats.data(), 1, dist.data(), ids.data());
            float raw = *std::max_element(dist.begin(), dist.end());
            infer_ms += t.toc();

            std::string dec = (raw > THRESH ? "Anomaly" : "Normal");
            csv << f.filename().string() << ',' << raw << ',' << dec << '\n';
        }
        csv.close();
        ...
        std::cout << "I/O time          : " << io_ms   << " ms\n"
                  ...
                  << "Avg. inference/img: "
                  << (images.empty() ? 0 : infer_ms / images.size()) << " ms\n"
                  << "result_patchcore.csv saved\n";
